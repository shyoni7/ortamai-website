...
export default function GradientButton({ children, onClick, href, className = '', size = 'md' }: GradientButtonProps) {
  ...
  const gradLayerStyle: React.CSSProperties = {
    ...
    width: '500%',
    aspectRatio: '1',
    background: 'radial-gradient(ellipse at 65% 180%, #C8C8D0, #0F172A, #08080C, #0F172A, #1A1A22, #0F172A, #08080C)',
  ...
  const darkOverlayStyle: React.CSSProperties = {
    ...
    inset: 0,
    borderRadius: '9999px',
    background: 'rgba(13, 18, 40, 0.55)',   /* semi-transparent dark purple */
    pointerEvents: 'none',
    zIndex: 1,
  };
    ...
    <div style={wrapperStyle} className={`grad-btn-wrapper ${className}`}>
      <style>{`
        ...
        .grad-btn-wrapper:hover {
          ...
          box-shadow: 0 0 16px 3px rgba(200,200,208,0.4);
        }
